// SpiralAnimation.jsx
// 2D canvas spiral-galaxy particle burst. Ported from a Next/TS source to JSX.
// Used as a one-time full-screen intro that plays after the loader, then fades
// into the site. Pass `loop`/`duration`/`onFinish` to control playback.
...
class AnimationController {
  constructor(canvas, ctx, dpr, size, opts = {}) {
    this.canvas = canvas;
    this.ctx = ctx;
    this.dpr = dpr;
    this.size = size;
    ...
    this.trailLength = 80;
    this.particleColor = opts.particleColor || "white";
    this.bgColor = opts.bgColor || "black";
...
export default function SpiralAnimation({
  ...
  }, [dimensions, loop, duration, particleColor, bgColor, onFinish]);
